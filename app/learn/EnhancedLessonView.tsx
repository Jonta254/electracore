...
function QuantityDiagram({ courseSlug, lessonId }: { courseSlug: string; lessonId: string }) {
  ...
  if (courseSlug === "solar-pv") {
    ...
    const label = n <= 5 ? "PV I-V CURVE AND OPERATING POINTS" : n <= 10 ? "ARRAY STRING AND YIELD DESIGN" : n <= 15 ? "INVERTER PORTS AND MPPT PATHS" : n <= 20 ? "BATTERY ENERGY AND SAFETY PATHS" : "PV SYSTEM ENERGY PATH";
