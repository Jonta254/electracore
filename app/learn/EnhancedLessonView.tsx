...
function QuantityDiagram({ courseSlug, lessonId }: { courseSlug: string; lessonId: string }) {
  ...
  if (courseSlug === "inspection-testing") {
    const n=Number(lessonId.slice(1)); const label=n<=5?"SAFE TEST SEQUENCE AND EVIDENCE":n<=10?"CONTINUITY TOPOLOGY AND PATTERN":n<=15?"INSULATION TEST BOUNDARY AND LEAKAGE":n<=20?"EARTH-FAULT LOOP AND DISCONNECTION":n<=25?"RCD FIELD VERIFICATION":n<=29?"POLARITY AND PROSPECTIVE FAULT CURRENT":"INSTALLATION VERIFICATION PATH";
