  ...
  "domestic-wiring": {
    ...
    modules: [
      ...
      },
      {
        ...
        lessons: [
          { id: "l36", title: "BS 7671 structure and using the regs", duration: "8min", type: "lesson" },
          { id: "l37", title: "Notification, certification and sign-off", duration: "8min", type: "lesson" },
          { id: "l38", title: "Part P and building control", duration: "7min", type: "lesson" },
  ...
  "protection-fault-analysis": {
    ...
    modules: [
      ...
      { id: "m5", title: "Discrimination & Coordination", duration: "35min", lessons: [
        { id: "l21", title: "What is discrimination and why it matters", duration: "8min", type: "lesson" },
        { id: "l22", title: "Current discrimination", duration: "7min", type: "lesson" },
        { id: "l23", title: "Time discrimination", duration: "7min", type: "lesson" },
        { id: "l24", title: "Energy discrimination for MCBs", duration: "8min", type: "lesson" },
      ...
      { id: "m6", title: "Testing for Protection", duration: "38min", lessons: [
        { id: "l26", title: "Testing Zs with loop testers", duration: "9min", type: "lesson" },
        { id: "l27", title: "Measuring PFC at the board", duration: "8min", type: "lesson" },
        { id: "l28", title: "RCD trip time testing", duration: "9min", type: "lesson" },
  ...
  "three-phase-systems": {
    ...
    modules: [
      ...
      { id: "m4", title: "Three-Phase Power", duration: "40min", lessons: [
        { id: "l14", title: "Active, reactive, and apparent power", duration: "9min", type: "lesson" },
        { id: "l15", title: "Power factor in three-phase", duration: "9min", type: "lesson" },
        { id: "l16", title: "Two-wattmeter method", duration: "10min", type: "lesson" },
        { id: "l17", title: "Power correction capacitor sizing", duration: "7min", type: "lesson" },
      ...
      { id: "m6", title: "Transformers", duration: "40min", lessons: [
        ...
        { id: "l25", title: "Turns ratio and voltage/current transformation", duration: "9min", type: "lesson" },
        { id: "l26", title: "Three-phase transformer connections", duration: "10min", type: "lesson" },
  ...
  "cable-sizing": {
    ...
    modules: [
      ...
      { id: "m2", title: "Derating & Correction Factors", duration: "50min", lessons: [
        { id: "l6", title: "Ambient temperature correction (Ca)", duration: "9min", type: "lesson" },
        { id: "l7", title: "Grouping correction factor (Cg)", duration: "10min", type: "lesson" },
        { id: "l8", title: "Thermal insulation factor (Ci)", duration: "9min", type: "lesson" },
        { id: "l9", title: "Depth of burial correction (Cs)", duration: "8min", type: "lesson" },
  ...
  "inspection-testing": {
    ...
    modules: [
      ...
      { id: "m4", title: "Earth Fault Loop Impedance", duration: "40min", lessons: [
        { id: "l16", title: "External impedance Ze measurement", duration: "9min", type: "lesson" },
        { id: "l17", title: "Total loop impedance Zs measurement", duration: "9min", type: "lesson" },
        { id: "l18", title: "Comparing Zs to Table 41.1 limits", duration: "9min", type: "lesson" },
        { id: "l19", title: "Correcting excessive Zs readings", duration: "8min", type: "lesson" },
...
export interface Lesson { id: string; title: string; duration: string; type: "lesson" | "quiz" | "exercise"; }
export interface Module { id: string; title: string; duration: string; lessons: Lesson[]; }
...
function getQuizForLesson(title: string, slug: string): QuizQuestion | null {
  ...
  if (slug === "led-lighting" && t.includes("driver circuit quiz")) {
    ...
    return { question: "Which statement matches the current BS 7671 field-verification approach for a general non-delay RCD?", options: ["Use the required AC test at rated IΔn and applicable time limit; ½×, 5×, ramp and type-specific tests are additional diagnostics unless another requirement applies", "Every RCD must pass ½×, 1× and 5× tests", "Only a 5× test proves additional protection", "A fast result proves the RCD waveform type is correctly selected"], correct: 0, explanation: "Current field testing is intentionally simplified. Product type, selection, delay, wiring and protective purpose remain separate checks." };
  }
  if (slug === "inspection-testing" && t.includes("polarity and pfc quiz")) {
    return { question: "PFC is 8.4 kA and a breaker is marked 6 kA. What makes the arrangement acceptable?", options: ["An exact documented upstream backup/cascading combination or reselected equipment/assembly rated for the fault current", "A second 6 kA breaker in series because ratings add", "An RCD trip-time test", "A low R1+R2 reading"], correct: 0, explanation: "Breaking capacities do not add. Only exact manufacturer-declared backup conditions or adequately rated equipment prove safe fault interruption." };
  }  if (slug === "inspection-testing" && t.includes("ir testing quiz")) {
    return { question: "Five isolated healthy branches each measure 20 MΩ to earth. What combined IR is expected when all five are connected in parallel, and what does it mean?", options: ["About 4 MΩ; parallel leakage lowers the combined value, which still needs comparison with applicable criteria and trends", "100 MΩ; insulation resistances always add in series", "20 MΩ; the lowest branch fixes the total", "0 MΩ; any parallel circuit is a fault"], correct: 0, explanation: "For five equal parallel resistance paths, Rtotal=R/5=4 MΩ. Configuration, test voltage, equipment and required minimum still matter." };
  }  if (slug === "inspection-testing" && t.includes("preparation quiz")) {
    return { question: "Protective-conductor continuity fails on a new circuit. What is the correct next action?", options: ["Keep isolated, locate and correct the defect, repeat affected dead tests, then reconsider live tests", "Measure live Zs and accept it if below a table value", "Record N/A for continuity", "Energize briefly to see whether the protective device trips"], correct: 0, explanation: "The protective fault path is a prerequisite for later energised testing. A later measurement cannot justify knowingly energising an unproved path." };
  }  if (slug === "industrial-control" && t.includes("final assessment")) {
    return { question: "A panel powers up, its PLC reports no faults, and the motor runs. What still proves completion?", options: ["Assembly/design verification, point-to-point I/O, sequence and fault modes, every safety function, power recovery, controlled as-builts and closed defects", "The green PLC RUN LED", "Only protective-conductor continuity", "A copy of the unmodified design drawing"], correct: 0, explanation: "Normal operation is one test state. Commissioning must prove the physical assembly, I/O, software, abnormal states, safety functions and controlled documentation." };
  }  if (slug === "industrial-control" && t.includes("safety systems quiz")) {
    return { question: "A guard switch and emergency stop feed a safety PLC. What establishes the required safety performance?", options: ["Validated sensor-logic-output functions, architecture/reliability/diagnostics, response and machine safe state against the risk-derived PLr or SIL", "The safety PLC certificate alone", "Two input wires regardless of common-cause faults", "A successful ordinary stop command"], correct: 0, explanation: "Functional safety belongs to each complete safety function. Certified components help, but integration, calculations, fault behavior and validation prove the achieved performance." };
...
function getExerciseForLesson(title: string, _slug: string): { problem: string; steps: string[]; answer: string } | null {
  ...
  if (_slug === "inspection-testing" && t.includes("full mock test schedule")) {
    return { problem: "A mock schedule shows CPC continuity failed, IR was left blank, live Zs is low through parallel pipework, a general RCD passed an obsolete 5x test but failed the required 1x test, and PFC exceeds an undocumented breaker rating. Complete the disposition.", steps: ["Declare the schedule incomplete/unsatisfactory: a failed CPC and missing IR prevent safe progression; blank is not N/A.", "Reject the low parallel-path Zs as replacement for the designed CPC; repair and repeat continuity/polarity/IR before justified live tests.", "The required rated-current RCD failure is not rescued by the optional 5x result; investigate wiring/device and retest under current method.", "Resolve PFC versus exact device/assembly breaking or documented backup rating, then issue the appropriate certificate/report with defects, corrective evidence and all repeated results."], answer: "Do not certify satisfactory: isolate/repair, complete missing evidence, resolve RCD and fault-rating failures, repeat dependent tests and document truthfully" };
  }  if (_slug === "inspection-testing" && t.includes("zs measurement exercise")) {
    return { problem: "A circuit has Ze=0.24 Ω and corrected R1+R2=0.51 Ω. Live Zs measures 0.31 Ω. The supplied device requires Ia=400 A at U0=230 V. Assess the evidence.", steps: ["Calculated designed-path Zs≈0.24+0.51=0.75 Ω.", "The device relationship gives supplied Zs,max=230/400=0.575 Ω, so the calculated path fails; do not round or use the low live value alone.", "The live 0.31 Ω result is far below 0.75 Ω and likely includes parallel paths or method effects; it cannot replace CPC continuity/design evidence.", "Keep the circuit from service as required, reconcile instrument and paths, inspect/repair or redesign the internal protective path, then repeat affected continuity, polarity, IR and loop checks."], answer: "The designed path fails the supplied 0.575 Ω maximum; the 0.31 Ω parallel-influenced live reading does not rescue it" };
  }  if (_slug === "inspection-testing" && t.includes("continuity exercise")) {
    return { problem: "A ring final has r1=0.80 Ω, rn=0.82 Ω and r2=1.32 Ω. L-N cross-connected readings cluster around 0.405 Ω. L-CPC readings rise from about 0.53 Ω toward 0.66 Ω near the midpoint then fall. One outlet reads open circuit on L-CPC. Interpret the evidence.", steps: ["The simple balanced L-N expectation is (0.80+0.82)/4=0.405 Ω, matching the cluster and supporting the line/neutral ring topology.", "Because line and CPC end-to-end resistances differ, L-CPC readings need not be identical; a rise toward midpoint and fall can be the expected Step 3 pattern.", "The single open outlet is not part of that normal pattern: keep isolated and inspect its CPC/contact, spur and terminal identity.", "After correction, repeat the affected ring/polarity tests, restore all ends and record r1/rn/r2 plus point-pattern evidence."], answer: "The main ring pattern is plausible, but the open L-CPC outlet is a defect requiring correction and repeated continuity/polarity evidence before energisation" };
  }  if (_slug === "industrial-control" && t.includes("writing a simple motor control program")) {
    return { problem: "Design scan-based logic for a motor with momentary START, healthy STOP/overload permissive, seal-in, no automatic restart after power recovery, and a 3 s delayed auxiliary fan. Safety stopping remains hardwired/safety-rated outside this basic PLC logic.", steps: ["Create one owned RunRequest state: set only on a rising START edge while every permissive is healthy; reset with priority on STOP, overload, mode loss or first scan/power recovery.", "Drive MotorCommand from RunRequest AND all current operational permissives; do not write the output from multiple rungs.", "Start a TON when proved motor-running feedback is true; command the fan only after its 3 s done state and reset it immediately when motor proof is lost.", "Test held START through power-up, STOP/overload, contactor failure, feedback loss, timer reset, manual/auto transitions and document that safety functions use their separately validated path."], answer: "Use reset-priority, edge-triggered single-owner state; no power-up restart; delayed fan follows proved running feedback, with safety outside ordinary PLC logic" };
  }  if (_slug === "industrial-control" && t.includes("diagram reading exercises")) {
    return { problem: "A forward/reverse drawing has electrical cross-interlocks, but no mechanical interlock. Its PLC can request both directions, and reverse is permitted immediately after forward drops. Review the design.", steps: ["Trace the main circuit: reversing swaps two phases, so simultaneous main-contact closure can create a severe fault.", "Electrical NC cross-interlocks block normal opposing coil commands, but analyze welded auxiliaries/main contacts and PLC/common-cause commands.", "Add the manufacturer-approved mechanical interlock where required and ensure contactors have the utilization duty for reversal/jogging.", "Define stop/release/verified delay before reversal, restart behavior, limit/permissive logic and validate all normal and fault states."], answer: "The drawing is incomplete: retain electrical interlocks, add the required mechanical/fault controls and a validated stop-before-reverse sequence" };
  }  if (_slug === "industrial-control" && t.includes("contactor circuit problems")) {
    return { problem: "A 12 A motor is jogged frequently. A contactor is rated 25 A AC-3 but 9 A AC-4. Its 230 V AC coil was mistakenly specified for a 24 V DC control supply. Identify the design decisions.", steps: ["Frequent jogging is the severe supplied AC-4 duty; compare the 12 A motor with the 9 A AC-4 rating, not the 25 A AC-3 marking.", "Because 12 A exceeds 9 A, reject the contactor and select a coordinated device with adequate declared AC-4 current and operating frequency/life.", "The 230 V AC coil is incompatible with the 24 V DC control supply; select the exact correct coil/control product and suppression method.", "Then verify short-circuit protective combination, overload setting/class, auxiliaries/interlocks, enclosure derating, fault rating, restart behavior and safe isolation."], answer: "Reject the proposed contactor/coil: its AC-4 duty is undersized and its coil supply is incompatible; reselect and coordinate the complete starter" };
...
function RCDBalanceDiagram() {
  return (
    <svg viewBox="0 0 400 160" style={ldWrap} role="img" aria-label="RCD core-balance principle">
      ...
      <text x="226" y="106" fontFamily="monospace" fontSize="7" fill={LD.d}>time limit depends on device type and test current</text>
      <text x="226" y="122" fontFamily="monospace" fontSize="7" fill={LD.d}>verify current rules and manufacturer instructions</text>
    </svg>
  );
}
...
function ThreePhaseWaveDiagram() {
  ...
  return (
    <svg viewBox="0 0 400 160" style={ldWrap} role="img" aria-label="Three-phase sine waves and star/delta connections">
      ...
      <text x="288" y="91" fontFamily="monospace" fontSize="7.5" fill={LD.t}>Y: Vʟ=√3·Vₚ</text>
      <text x="288" y="103" fontFamily="monospace" fontSize="7.5" fill={LD.t}>Δ: Iʟ=√3·Iₚ</text>
      {/* delta */}
...
function MCBCurveDiagram() {
  return (
    <svg viewBox="0 0 400 160" style={ldWrap} role="img" aria-label="MCB trip curves for types B, C and D">
      <text x="14" y="16" fontFamily="monospace" fontSize="9" fill={LD.c} opacity="0.6">MCB OPERATING REGIONS · SCHEMATIC ONLY</text>
      {/* axes */}
      ...
      <text x="340" y="134" fontFamily="monospace" fontSize="7" fill={LD.d}>multiple of In →</text>
      {/* thermal curve */}
      ...
      <text x="92" y="150" fontFamily="monospace" fontSize="7.5" fill={LD.t} opacity="0.8">use the manufacturer time-current curve for coordination</text>
    </svg>
  );
}
...
function ACWaveDiagram() {
  ...
  return (
    <svg viewBox="0 0 400 160" style={ldWrap} role="img" aria-label="AC sine wave with peak and RMS values">
      ...
      {/* RMS is an equivalent heating value, not a second instantaneous waveform. */}
      <line x1="24" y1="80" x2="24" y2="49" stroke={LD.g} strokeWidth="1.2" /><path d="M20 53 L24 49 L28 53 M20 76 L24 80 L28 76" fill="none" stroke={LD.g} strokeWidth="1" />
      <text x="30" y="58" fontFamily="monospace" fontSize="7" fill={LD.g}>RMS magnitude</text>
      <text x="54" y="140" fontFamily="monospace" fontSize="8.5" fill={LD.t} opacity="0.85">sine wave: Vᵣₘₛ = Vₚₖ / √2   ·   T = 1/f</text>
    </svg>
  );
}
...
function CableDeratingDiagram() {
  return (
    <svg viewBox="0 0 400 160" style={ldWrap} role="img" aria-label="Cable current capacity reduced by derating factors">
      ...
      {[{ x: 20, t: "In", s: "device", c: LD.v }, { x: 104, t: "÷ Ca", s: "ambient", c: LD.c }, { x: 182, t: "÷ Cg", s: "grouping", c: LD.c }, { x: 260, t: "÷ Ci", s: "insul.", c: LD.c }].map((b, i) => (
        <g key={i}>
          ...
          {i < 3 && <text x={b.x + 76} y="76" fontFamily="monospace" fontSize="11" fill={LD.d}>→</text>}
        </g>
      ))}
      <text x="340" y="76" fontFamily="monospace" fontSize="10" fill={LD.g}>= It</text>
      <rect x="60" y="112" width="280" height="30" rx="5" fill="rgba(52,211,153,0.05)" stroke="rgba(52,211,153,0.2)" strokeWidth="0.7" />
      <text x="200" y="131" textAnchor="middle" fontFamily="monospace" fontSize="8.5" fill={LD.t}>select cable with tabulated capacity ≥ It; include all applicable factors</text>
    </svg>
  );
}
...
function PVSystemDiagram() {
  return (
    <svg viewBox="0 0 400 160" style={ldWrap} role="img" aria-label="Solar PV system from panel to grid and battery">
      ...
      <text x="218" y="118" fontFamily="monospace" fontSize="6.5" fill={LD.d}>optional compatible storage port</text>
...
function DOLStarterDiagram() {
  return (
    <svg viewBox="0 0 400 160" style={ldWrap} role="img" aria-label="Direct-on-line motor starter control circuit">
      ...
      {/* overload NC and start NO */}
      <line x1="82" y1="52" x2="102" y2="52" stroke={LD.c} strokeWidth="1.6" /><line x1="102" y1="46" x2="102" y2="58" stroke={LD.h} strokeWidth="1.2" /><line x1="114" y1="46" x2="114" y2="58" stroke={LD.h} strokeWidth="1.2" /><line x1="102" y1="52" x2="114" y2="46" stroke={LD.h} strokeWidth="1" /><text x="100" y="72" fontFamily="monospace" fontSize="6.5" fill={LD.d}>OL NC</text>
      <line x1="114" y1="52" x2="132" y2="52" stroke={LD.c} strokeWidth="1.6" /><line x1="132" y1="46" x2="132" y2="58" stroke={LD.g} strokeWidth="1.2" /><line x1="144" y1="46" x2="144" y2="58" stroke={LD.g} strokeWidth="1.2" /><text x="128" y="72" fontFamily="monospace" fontSize="6.5" fill={LD.d}>START NO</text>
      {/* coil */}
      <line x1="144" y1="52" x2="300" y2="52" stroke={LD.c} strokeWidth="1.6" /><circle cx="316" cy="52" r="12" fill="rgba(240,165,0,0.12)" stroke={LD.c} strokeWidth="1.4" /><text x="316" y="56" textAnchor="middle" fontFamily="monospace" fontSize="9" fill={LD.c}>KM1</text><line x1="328" y1="52" x2="360" y2="52" stroke={LD.c} strokeWidth="1.6" />
      {/* hold-in aux */}
      <line x1="114" y1="52" x2="114" y2="90" stroke={LD.c} strokeWidth="1.4" /><line x1="132" y1="84" x2="132" y2="96" stroke={LD.c} strokeWidth="1.2" /><line x1="144" y1="84" x2="144" y2="96" stroke={LD.c} strokeWidth="1.2" /><line x1="114" y1="90" x2="132" y2="90" stroke={LD.c} strokeWidth="1.4" /><line x1="144" y1="90" x2="154" y2="90" stroke={LD.c} strokeWidth="1.4" /><line x1="154" y1="90" x2="154" y2="52" stroke={LD.c} strokeWidth="1.4" /><text x="118" y="108" fontFamily="monospace" fontSize="7" fill={LD.d}>KM1 aux NO (seal-in)</text>
      {/* motor */}
      ...
      <text x="134" y="140" fontFamily="monospace" fontSize="8" fill={LD.t} opacity="0.75">control rung shown; power poles and motor protection are separate</text>
    </svg>
  );
}
...
function AtomDiagram() {
  return (
    <svg viewBox="0 0 400 160" style={ldWrap} role="img" aria-label="Atom with electron shells and drifting free electron">
      <text x="14" y="16" fontFamily="monospace" fontSize="9" fill={LD.c} opacity="0.6">SIMPLIFIED CHARGE MODEL · I = Q / t</text>
...
function EarthingMiniDiagram() {
  return (
    <svg viewBox="0 0 400 160" style={ldWrap} role="img" aria-label="TN-C-S and TT earthing arrangements">
      ...
      <text x="274" y="134" fontFamily="monospace" fontSize="7.5" fill={LD.d}>installation electrode; verify ADS and RCD requirements</text>
    </svg>
  );
}

function TestSequenceDiagram() {
  const dead = ["Protective-conductor continuity", "Ring continuity (if present)", "Insulation resistance", "Polarity"];
  const conditional = ["Earth electrode (if present)", "Ze / Zs or verified enquiry", "RCD operation", "Phase sequence / function"];
  return (
    <svg viewBox="0 0 480 190" style={ldWrap} role="img" aria-label="Typical initial verification sequence: relevant tests before energization, followed by method-dependent and functional verification">
      <text x="14" y="16" fontFamily="monospace" fontSize="9" fill={LD.v} opacity="0.6">INITIAL VERIFICATION · SELECT TESTS FOR THE INSTALLATION</text>
      <rect x="16" y="30" width="216" height="126" rx="6" fill="rgba(52,211,153,0.05)" stroke={LD.g} strokeWidth="0.9" />
      <text x="26" y="48" fontFamily="monospace" fontSize="8" fill={LD.g}>BEFORE ENERGIZATION (WHERE RELEVANT)</text>
      {dead.map((t, i) => (<g key={t}><rect x="26" y={58 + i * 22} width="196" height="17" rx="4" fill="#12151A" stroke={LD.g} strokeWidth="0.7" /><text x="34" y={70 + i * 22} fontFamily="monospace" fontSize="7.2" fill={LD.t}>{i + 1}. {t}</text></g>))}
      <polygon points="236,89 250,95 236,101" fill={LD.d} />
      <rect x="254" y="30" width="210" height="126" rx="6" fill="rgba(240,165,0,0.05)" stroke={LD.c} strokeWidth="0.9" />
      <text x="264" y="48" fontFamily="monospace" fontSize="8" fill={LD.c}>METHOD / SYSTEM DEPENDENT</text>
      {conditional.map((t, i) => (<g key={t}><rect x="264" y={58 + i * 22} width="190" height="17" rx="4" fill="#12151A" stroke={LD.c} strokeWidth="0.7" /><text x="272" y={70 + i * 22} fontFamily="monospace" fontSize="7.2" fill={LD.t}>{i + 5}. {t}</text></g>))}
      <text x="18" y="176" fontFamily="monospace" fontSize="7" fill={LD.d}>Typical order only: stop on unsatisfactory results; use the current procedure, instrument instructions, and risk controls.</text>
    </svg>
  );
}
...
function LEDLuxDiagram() {
  return (
    <svg viewBox="0 0 400 160" style={ldWrap} role="img" aria-label="LED luminaire illuminating a work plane, lumen method">
      ...
      <text x="345" y="66" textAnchor="middle" fontFamily="monospace" fontSize="8" fill={LD.t}>Eav = N·F·UF·MF</text>
  ...
  return (
    <>
                              ...
                              <div key={lesson.id}>
                                <div className={`lesson-item${isActive ? " active" : ""}${isDone ? " done" : ""}`}>
                                  ...
                                  <Link
                                    ...
                                    onClick={() => { try { saveLastLesson(localStorage, slug, lesson.id); } catch {} }}
                                  >
      ...
      <style>{`
        ...
        .curriculum-continue { display: inline-flex; align-items: center; min-height: 38px; padding: 0 1rem; border-radius: 8px; border: 1px solid rgba(240,165,0,.35); background: rgba(240,165,0,.1); color: var(--core); font-size: .78rem; font-weight: 750; text-decoration: none; }
