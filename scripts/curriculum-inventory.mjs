...
const enhancementSource = read("../app/learn/enhancedLessons.ts") + read("../app/learn/fundamentalsGroupOne.ts") + read("../app/learn/fundamentalsGroupTwo.ts")
  ...
  + read("../app/learn/solarInverterBatteryLessons.ts")
  + read("../app/learn/solarGridCommissioningLessons.ts")
  + read("../app/learn/industrialStarterProtectionLessons.ts")
  + read("../app/learn/industrialDiagramSafetyLessons.ts")
  + read("../app/learn/industrialPlcPanelLessons.ts")
  + read("../app/learn/inspectionPreparationContinuityLessons.ts")
  + read("../app/learn/inspectionIrLoopLessons.ts")
  + read("../app/learn/inspectionRcdPfcLessons.ts")
  + read("../app/learn/inspectionCertificationLessons.ts")
  + read("../app/learn/ledTechnologyDriverLessons.ts")
  + read("../app/learn/ledEmergencyDesignLessons.ts");
...
const counts={ enhanced:rows.filter(r=>r.enhanced).length, recorded:rows.filter(r=>r.review).length, unrecorded:rows.filter(r=>!r.review).length, professionalPending:rows.filter(r=>r.review?.state==="enhanced-review-pending").length };
const header=["# Curriculum matrix","",`Generated from the preserved course database on ${generatedOn}. Courses: ${new Set(rows.map(r=>r.course)).size}; lessons: ${rows.length}; structured enhancements: ${counts.enhanced}; lessons with editorial or calculation-review evidence: ${counts.recorded}; lessons without a review record: ${counts.unrecorded}; enhanced lessons awaiting credentialed professional approval: ${counts.professionalPending}.`,"","A review record documents the stated editorial, source, calculation, or assessment checks. It is not credentialed professional approval. Professional approval requires a separate identity, credential, date, and exact content/source hashes in `app/learn/contentApproval.ts`; none are currently recorded.","","Lesson IDs are compatibility-sensitive internal identifiers. Review states come only from `app/learn/reviewStates.ts`; absent entries are explicitly incomplete.","","| Course | Module | Lesson | Existing URL / ID | Duration | Type | Content record | Review state | Evidence |","| --- | --- | --- | --- | --- | --- | --- | --- | --- |"]; const esc=v=>String(v).replaceAll("|","\\|").replaceAll("\n"," ");
...
console.log(`Wrote ${rows.length} rows: ${counts.enhanced} enhanced, ${counts.recorded} with review evidence, ${counts.unrecorded} unrecorded, ${counts.professionalPending} awaiting professional approval.`);
